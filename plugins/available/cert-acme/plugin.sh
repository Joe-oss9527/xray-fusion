...
cert_acme::configure_pre() {
  ...
  [[ -n "${domain}" ]] || {
    echo "[cert-acme] XRAY_DOMAIN not set; skip" >&2
    return 0
  }
  if acme_sh::exists "${dir}"; then
    echo "[cert-acme] cert exists in ${dir}" >&2
    return 0
  fi
  if [[ "${XRF_DRY_RUN:-false}" == "true" ]]; then
    echo "[cert-acme] would issue cert for ${domain} into ${dir}" >&2
    mkdir -p "${dir}"
    : > "${dir}/fullchain.pem"
    : > "${dir}/privkey.pem"
    return 0
  fi
  local email="${XRAY_EMAIL:-admin@${domain}}"
  echo "[cert-acme] issuing cert for ${domain} -> ${dir}" >&2
  acme_sh::issue "${domain}" "${email}" "${dir}" || {
    echo "[cert-acme] issue failed" >&2
    return 1
  }
}
