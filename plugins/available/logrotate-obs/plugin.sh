...
logrotate_obs::configure_post() {
  ...
  if [[ "${target}" == "file" ]]; then
    ...
    : > "${d}/access.log" 2> /dev/null || true
    : > "${d}/error.log" 2> /dev/null || true
    chown root:xray "${d}/"*.log 2> /dev/null || true
    chmod 0640 "${d}/"*.log 2> /dev/null || true
...
logrotate_obs::service_setup() {
  ...
  if [[ "${target}" == "file" ]]; then
    _build_logrotate_body | io::atomic_write "$(_logrotate_path)" 0644
    core::log info "[logrotate-obs] logrotate installed" "$(printf '{"path":"%s"}' "$(_logrotate_path)")"
  fi
  if [[ "${XRF_JOURNAL_TUNE:-false}" == "true" ]]; then
    io::ensure_dir "$(_journal_dropin_dir)" 0755
    {
      echo "[Journal]"
      [[ -n "${XRF_JOURNAL_MAX:-}" ]] && echo "SystemMaxUse=${XRF_JOURNAL_MAX}"
      echo "RateLimitIntervalSec=${XRF_JOURNAL_RATE_INTERVAL:-30s}"
      echo "RateLimitBurst=${XRF_JOURNAL_RATE_BURST:-1000}"
    } | io::atomic_write "$(_journal_dropin)" 0644
    systemctl restart systemd-journald 2> /dev/null || true
    core::log info "[logrotate-obs] journald tuned" "$(printf '{"dropin":"%s"}' "$(_journal_dropin)")"
  fi
}
...
logrotate_obs::links_render() {
  ...
  if [[ "${target}" == "file" ]]; then
    echo "File logs: $(_log_dir)/{error.log,access.log}"
    echo "Follow:    tail -F $(_log_dir)/error.log"
    echo "Rotate:    sudo logrotate -f $(_logrotate_path)"
  else
    echo "Journald:  sudo journalctl -u xray -n 200 -f"
    echo "Filter:    sudo journalctl -u xray --since '1 hour ago' | grep -i error"
  fi
  echo "[obs] ================================="
}
